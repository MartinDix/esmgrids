...
class UMGrid_nd(BaseGrid):
    ...
    def __init__(self, grid, mask_file=None):
        ...
        description = f"UM ND {grid}"

        reg = re.compile('n(\d+)_([tuv])')
        m = reg.search(grid)
        if not m:
        ...
        # Set lats and lons.
        lon_t = np.linspace(0, 360, nlon_t, endpoint=False)
        lat_t = np.linspace(-90, 90, nlat_t)

        lat_u = lat_t
        lon_u = dx_half + np.linspace(0, 360, nlon_t, endpoint=False)

        lon_v = lon_t
        lat_v = dy_half + np.linspace(-90, 90, nlat_v)
